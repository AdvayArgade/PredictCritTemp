...
top_features = list(results$optVariables[1:10], top_attrs[1:10], FS_features[1:10], common_features[1:10])
algorithms = list('RFE', 'Information Gain', 'Fischer Score', 'RFE intersection Information Gain')
for(i in length(top_features)){
  cat("\nTop 10 features of ",algorithms[i], ":")
  # XGBoost
  cat("\nTop 10 features:")
  set.seed(42)
  top_10_features = top_features[i]
  ...
  data_filtered <- A[, c("critical_temp", top_10_features)]
  ...
  data_filtered <- data[, c("critical_temp", top_10_features)]
